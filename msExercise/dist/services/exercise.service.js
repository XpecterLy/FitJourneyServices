...
const GetAllExerciseService = (muscleGroupId, trainingStyleId) => __awaiter(void 0, void 0, void 0, function* () {
    var filter = {};
    if (muscleGroupId != undefined)
        filter.muscleGroupId = muscleGroupId;
    if (trainingStyleId != undefined)
        filter.trainingStyleId = trainingStyleId;
    const res = yield exercise_schemas_1.exerciseModel.find(filter);
    return res.map(item => ({
        id: item.id,
        name: item.name,
