...
export const GetAllCategoriesMuscleGroupService = async(trainingStylesId?: string, limit?: number): Promise<CategoriesMuscleGroupType[]> => {
    var filter = {};
    ...
    const res = await categoriesMuscleGroupModel.find(filter).limit(limit || 10);
